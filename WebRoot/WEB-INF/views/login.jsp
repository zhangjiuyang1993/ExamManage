...
<body>
	...
	<div class="header">
		<div id="messageBox" class="alert alert-error ${empty result ? 'hide' : ''}"><button data-dismiss="alert" class="close">×</button>
			<label id="loginError" class="error">${result}</label>
		</div>
	</div>
	<h1 class="form-signin-heading">瑞猿在线考试管理系统</h1>
	<form id="loginForm" class="form-signin" action="j_spring_security_check" method="post">
		<label class="input-label" for="username">登录名</label>
		<input type="text" id="j_username" name="j_username" class="input-block-level required" value="${username}">
		<label class="input-label" for="password">密码</label>
		<input type="password" id="j_password" name="j_password" class="input-block-level required">
		<%--
